...
const simple_twitter = new SimpleTwitter(JSON.parse(Deno.env.get('TWITTER')));
...
const fetchAPI = () => {
  ...
  simple_twitter.get("search/tweets", params, async function (
    ...
    if (!error) {
      ...
      while (!encontrado && indice < fetchedTweets.length) {
        ...
        if (posicionBs !== -1) {
          const precio = texto.substr(posicionBs + 4, texto.indexOf("\n") - 2);
  ...
  console.log("finished fetching, sleeping for 30 min...");
  setTimeout(fetchAPI, 1800000);
};
